# UseCase<!-- omit in toc -->


- [Administrator](#administrator)
- [Attendance and Expense Registration(AER)](#attendance-and-expense-registrationaer)
- [Attendance Management(AM)](#attendance-managementam)
- [Finance Management(FM)](#finance-managementfm)
- [User Management(UM)](#user-managementum)
- [Setting Management(SM)](#setting-managementsm)
- [Batch job](#batch-job)


## Administrator
![Project Logo](./image/administrator-usecase.drawio.png)

## Attendance and Expense Registration(AER)
![Project Logo](./image/AER-usecase.drawio.png)

## Attendance Management(AM)
![Project Logo](./image/AM-usecase.drawio.png)

## Finance Management(FM)
![Project Logo](./image/FM-usecase.drawio.png)

## User Management(UM)
![Project Logo](./image/UM-usecase.drawio.png)

## Setting Management(SM)
![Project Logo](./image/SM-usecase.drawio.png)

## Batch job
![Project Logo](./image/System-usecase.drawio.png)
