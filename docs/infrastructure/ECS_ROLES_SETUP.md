...
12. **「説明」** に `ECS task execution role for attendance app` と入力（任意）
13. **「ステップ 3: タグを追加する」** はスキップしてOK（タグは任意）
14. 画面下部の **「ロールを作成」** ボタンをクリック
...
4. **「Elastic Container Service タスク」** を選択（⚠️ **1つ目と同じユースケースです**）
5. **「次へ」** をクリック
...
8. **「説明」** に `ECS task role for attendance app` と入力（任意）
9. **「ステップ 3: タグを追加する」** はスキップしてOK（タグは任意）
10. 画面下部の **「ロールを作成」** ボタンをクリック
