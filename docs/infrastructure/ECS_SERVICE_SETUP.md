...
> **💡 重要**: 初回のALB作成時、AWSが自動的にサービスリンクロールを作成します。これは正常な動作です。
...
**ステップ 2: ネットワークマッピング**
5. **ネットワークマッピング**:
   - **VPC**: `vpc-08f1650afa45efb02`（デフォルトVPC）を選択
   - **アベイラビリティゾーン**: 
...
7. **セキュリティグループ**:
   ...
   - `sg-09e2841dce32a4347`（attendance-ecs-sg）にチェック
   - デフォルトのセキュリティグループは**チェックを外す**
8. **次へ** をクリック
...
9. **リスナーとルーティング**:
   - **プロトコル**: HTTP
   - **ポート**: 80
   - **デフォルトアクション**: **新しいターゲットグループを作成** を選択
10. **次へ** をクリック
...
**ステップ 6: 確認と作成**
13. **確認と作成** → **ロードバランサーを作成** をクリック
14. **「サービスリンクロールを作成しますか？」** というメッセージが表示された場合、**「作成」** をクリック（初回のみ）
15. ALBの作成が完了するまで数分待ちます（ステータスが「アクティブ」になるまで）
