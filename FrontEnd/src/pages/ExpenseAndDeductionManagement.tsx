...
const ExpenseAndDeductionManagement = () => {
  ...
  const handleExpenseClosure = async () => {
    ...
    try {
      ...
      const check = await fetch(`${process.env.REACT_APP_BASE_URL}api/check_expense_closure`, {
        method: "POST",
        mode: "cors",
      ...
      if (!check.ok || checkBody?.success === false) {
        ...
        const msg = names.length > 0
          ? `Cannot close expenses. Found non-eligible records (${breakdown}). Affected users: ${namePreview}${moreCount ? ` and ${moreCount} more` : ''}.`
          : (checkBody?.error || 'Cannot close expenses due to validation errors.');
